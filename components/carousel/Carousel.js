...
export const Carousel = (props) => (
  <div className="carousel">
    <button
      aria-label="Movimentar para esquerda"
      className="carousel__button carousel__button_left"
      onClick={() => movimentar("esquerda")}
    >
        ...
        {props.depoimentos.map((depoimento) => {
          return (
            <div
              ...
              key={`depoimento${props.depoimentos.indexOf(depoimento)}`}
            >
    ...
    <button
      aria-label="Movimentar para direita"
      className="carousel__button carousel__button_right"
      onClick={() => movimentar("direita")}
    >
...
const movimentar = (sinal) => {
  ...
  if (
    (posicaoEmNumero === 0 && sinal !== "direita") ||
    (posicaoEmNumero === quantidadeDeItens * larguraDoContainer * -1 &&
      sinal === "direita")
  ) {
  } else if (!posicao) {
    itens.style.left = `${
      sinal === "direita" ? "-" : ""
    }${larguraDoContainer}px`;
  } else {
    let novaPosicao =
      sinal === "direita"
